...
tabela_exemplo %>%
  gt() %>%
  tab_header(title = "Resumo dos centros de usinagem",
...
tabela_exemplo %>%
  gt() %>%
  tab_header(title = "Tabela com destaque visual") %>%
  cols_label(Centro_Usinagem = "Centro",
             Media_Diametro_mm = "Média (mm)",
             Desvio_Padrao_mm = "Desvio-padrão",
             Peca_Fora_Especificacao = "Peças fora",
             Percentual_Refugo = "Refugo (%)") %>%
  fmt_number(columns = c(Media_Diametro_mm, Desvio_Padrao_mm),
             decimals = 3) %>%
  data_color(columns = Percentual_Refugo, method = "numeric")

# Ordenação da tabela pelo maior percentual de refugo ----
tabela_ordenada <- tabela_exemplo %>%
  arrange(desc(Percentual_Refugo))

kable(tabela_ordenada,
